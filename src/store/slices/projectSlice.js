...
export const addNewProject = (data, user) => async (dispatch) => {
  dispatch(projectSlice.actions.addNewProjectRequest());
  try {
    const response = await axios.post(
      `${domain}/api/v1/project/add`,
      data,
      {
        withCredentials: true,
...
export const deleteProject = (id, user) => async (dispatch) => {
  dispatch(projectSlice.actions.deleteProjectRequest());
  try {
    const response = await axios.delete(
      `${domain}/api/v1/project/delete/${id}`,
      {
        withCredentials: true, 'Authorization': `${user.token}`,
      }
    );
...
export const updateProject = (id, newData, user) => async (dispatch) => {
  dispatch(projectSlice.actions.updateProjectRequest());
  try {
    const response = await axios.put(
      `${domain}/api/v1/project/update/${id}`,
      newData,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data", 'Authorization': `${user.token}`},
