...
export const addNewSoftwareApplication = ({data, user}) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${domain}/api/v1/softwareapplication/add`,
      data,
      {
        withCredentials: true,
...
export const deleteSoftwareApplication = ({id, user}) => async (dispatch) => {
  dispatch(
    softwareApplicationSlice.actions.deletesoftwareApplicationsRequest()
  );
  try {
    const response = await axios.delete(
      `${domain}/api/v1/softwareapplication/delete/${id}`,
      {
        withCredentials: true,
